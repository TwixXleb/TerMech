...
Drawed_Spiral_Spring = ax.plot(X_SpiralSpr + X_O, Y_SpiralSpr + Y_O, color='green')[0]  # Green for spring
...
Point_B = ax.plot(X_B, Y_B, marker='o', color='blue', markersize=12)[0]  # Blue for the ball
...
def Dordge(i):
    ...
    Point_O.set_data([X_O], [Y_O])  # Исправлено: передаем X_O и Y_O как последовательности
    Point_C.set_data([X_C[i]], [Y_C[i]])  # Исправлено: передаем X_C и Y_C как последовательности
    Point_A.set_data([X_A[i]], [Y_A[i]])  # Исправлено: передаем X_A и Y_A как последовательности
    Line_OC.set_data([X_O, X_C[i]], [Y_O, Y_C[i]])  # Линия от O до C
    Point_B.set_data([X_B[i]], [Y_B[i]])  # Исправлено: передаем X_B и Y_B как последовательности
    Line_AB.set_data([X_A[i], X_B[i]], [Y_A[i], Y_B[i]])  # Линия от A до B

    # Обновление спиральной пружины
...
anim = FuncAnimation(fig, Dordge, frames=1000, interval=20, blit=True)
